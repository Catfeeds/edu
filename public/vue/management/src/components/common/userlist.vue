  ...
  <div class="user-center">
    ...
    <el-dialog :close-on-click-modal="false" title="添加成员" :visible.sync="addUserByTable.addDialogTableVisible" width="1000px">
      ...
      <div slot="footer" class="dialog-footer">
        <el-pagination @current-change="addUserCurrentChange" background layout="prev, pager, next" :current-page="addUserByTable.pages.currentPage" :total="addUserByTable.pages.total">
        </el-pagination>
    ...
    fnReqUserInAdd () {
      ...
      if ((currentParams && currentParams.c === 'pro') || (currentParams && currentParams.c === 'cla')) {
        let params = {
          module: 'Service',
          controller: currentParams.c === 'pro' ? 'Profession_Con' : 'Class_Con',
          action: currentParams.c === 'pro' ? 'getProOtherUser' : 'getOtherClassUser',
          id: this.$route.query.d,
    ...
    addUserByTableSubmit () {
      ...
      if (this.addUserByTable.addUserList.length) {
        ...
        if ((currentParams && currentParams.c === 'pro') || (currentParams && currentParams.c === 'cla')) {
          ...
          if (currentParams.c === 'pro') {
            requestDta.controller = 'Profession_Con';
            requestDta.action = 'addBatchProUser';
            requestDta.id = this.$route.query.d;
            requestDta.user = ids;
          } else if (currentParams.c === 'cla') {
            requestDta.controller = 'Class_Con';
            requestDta.action = 'addClassmate';
            requestDta.class = this.$route.query.d;
            requestDta.id = ids;
          }
          this.$ajax.post(this.Const.apiurl, requestDta).then(res => {
            loading.close();
            let resData = res.data;
            if (!resData.status) {
              this.fnReqAllUser();
