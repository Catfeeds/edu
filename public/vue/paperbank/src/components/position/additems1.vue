...
export default {
  ...
  methods: {
    ...
    formatterQuesNum (row, column, cellValue, index) {
      ///console.log(1112222, row, column, cellValue, index);
    ...
    getQuestionData() {
      let params = {
        ...
        controller: 'Question',
        action: 'questionList',
        course: this.Const.courseData.query.course,
        classroom: '',
    ...
    getQuizDetail () {
      ...
      this.$ajax.post(this.Const.apiUrl, params).then(res => {
          let resData = res.data;
          console.log(8888888888, resData);
          if (resData.status == 0) {
    ...
    submitAddQues () {
      ...
      let params = {
        ...
        this.$ajax.post(this.Const.apiUrl, params).then(res => {
          let resData = res.data;
          if (resData.status == 0) {
            this.$message({type: 'info', message: resData.data ? '提交成功！' : '提交失败！'});
          } else {
    ...
    getQuestionNum () {
      let params = {
        ...
        id: this.Const.courseData.query.course
      };
      this.$ajax.post(this.Const.apiUrl, params).then(res => {
          let resData = res.data;
          if (resData.status == 0) {
