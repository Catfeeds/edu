    ...
    public function addQuestion() {
        ...
        switch ($data['qtype']) {
            ...
            case 'essay':
                ...
                break;
            case 'comprehensive':
                $add = $this->addComprehensive($data);//综合题
                break;
            ...
            for ($j = 0; $j < count($res_answer); $j++){
                if ($res_question['data'][$i]['id'] == $res_answer[$j]['questionid']){
                    switch ($res_question['data'][$i]['qtype']){
                        ...
                        //填空题
                        case 'shortanswer':
                            $res_question['data'][$i]['answer'] = explode('||', $res_answer[$j]['answer']);
                            break;
                        //判断题
                        case 'truefalse':
                            $res_question['data'][$i]['answer'][] = [
    ...
    */
    public function getQuestionNum(){
        ...
        if(@empty($data['id'])){
            ...
            $classroom = new Classroom();
            $courseid = $classroom->where(['status' => 0, 'id' => $data['id']])->value('course_id');//所传id为课堂id
            if (!$courseid) {
                $courseid = $data['id'];//所传id为课程id
            }
