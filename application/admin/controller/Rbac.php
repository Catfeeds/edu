...
class Rbac extends Common {
    ...
    public function searchUser() {
        if(input('key') != null) {
            ...
            $type = input('type');
            $user = new User;
            $config = ['query' => ['key' => $key, 'type' => $type]];
            if($type == "number"){
                $usermsg = $user->where('idnumber', 'like', '%' . $key . '%')->paginate(1, false, $config);
            }else if ($type == 'name') {
                $usermsg = $user->where('username', 'like', '%' . $key . '%')->paginate(1, false, $config);
            }
            $userrole = '';
            $page = '';
            if($usermsg) {
    ...
    /*
     *根据用户id获取该用户角色之下的所有角色
    */
    public function getSonRole($uid = false) {
        if (!$uid) {
            $uid = $this->data['uid'];
        }
        $role = new Role();
        $role_info = $role->getUserRole($uid);
        if (!$role_info) {
            echo getBack(1, '该用户无角色信息！');
            exit;
        }
        $where['level'] = ['gt', $role_info['level']];
        $info = $role->getRoleInfo($where, 'id, remark');
        if (!$info) {
            echo getBack(1, '该用户无可用子角色！');
            exit;
        }
        echo getBack(0, $info);
        exit;
